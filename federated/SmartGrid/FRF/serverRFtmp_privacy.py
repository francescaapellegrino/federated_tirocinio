...
print(f"🛡️ SERVER PRIVACY MODE: {PRIVACY_MODE.upper()}")
if ENABLE_DP:
    print(f"   ✅ Differential Privacy: ε={DP_EPSILON}")
if ENABLE_REGULARIZATION:
    print(f"   ✅ Model Regularization ATTIVA")
...
MAX_TREES_GLOBAL = 50                        # AUMENTATO: da 100 a 150
...
def main():
    ...
    print(f"🛡️ PRIVACY MECHANISMS:")
    print(f"  - Mode: {PRIVACY_MODE.upper()}")
    if ENABLE_DP:
        print(f"  - Differential Privacy: ε={DP_EPSILON}, δ={DP_DELTA}")
        print(f"    → Garantisce (ε,δ)-DP per protezione privacy formale")
    if ENABLE_REGULARIZATION:
        print(f"  - Model Regularization: n_est={REG_N_ESTIMATORS}, depth={REG_MAX_DEPTH}")
        print(f"    → Riduce overfitting per mitigare membership inference")
    if not (ENABLE_DP or ENABLE_REGULARIZATION):
        print(f"  - ⚠️ BASELINE MODE: Nessuna protezione privacy attiva")
    print("")
