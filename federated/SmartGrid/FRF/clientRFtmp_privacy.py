...
ENABLE_REGULARIZATION = PRIVACY_MODE in ['regularized', 'combined']

print(f"🛡️ PRIVACY MODE: {PRIVACY_MODE.upper()}")
if ENABLE_DP:
    print(f"   ✅ Differential Privacy: ε={DP_EPSILON}, δ={DP_DELTA}")
...
def create_random_forest_model(client_id):
    """
    Crea il modello Random Forest per SmartGrid CON PRIVACY MECHANISMS.
    Supporta baseline, regularized, DP, e combined modes.
    
    Returns:
        Modello RandomForestClassifier configurato con privacy mechanisms
    """
    print(f"[Client {client_id}] === CREAZIONE RANDOM FOREST CON PRIVACY MECHANISMS ===")
    print(f"[Client {client_id}] Privacy Mode: {PRIVACY_MODE}")
    
    # ✅ CONFIGURA PARAMETRI IN BASE ALLA MODALITÀ
    if ENABLE_REGULARIZATION:
        ...
        print(f"  - Min samples split: {min_samples_split} (aumentato)")
        print(f"  - Min samples leaf: {min_samples_leaf} (aumentato)")
    else:
    ...
    print(f"  - Random state: {client_random_state}")
...
def add_dp_noise_to_predictions(predictions, epsilon=DP_EPSILON):
    ...
    Returns:
        Predizioni con DP noise applicato
    """
    if not ENABLE_DP:
        return predictions
    ...
    try:
        ...
        print(f"[DP] ⚠️ Errore applicazione DP noise alle predizioni: {e}")
        return predictions
...
class SmartGridRandomForestClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        try:
            ...
            if ENABLE_DP:
                val_prob = add_dp_noise_to_predictions(val_prob, epsilon=DP_EPSILON)
