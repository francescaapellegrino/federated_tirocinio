...
class TrainingConfig:
    """
    Configurazione di addestramento per il client standalone.
    """
    def __init__(self):
        self.epochs_per_round = 5  # ⚠️ CONFIGURAZIONE: 5 epoche per round
        self.batch_size = 32
        self.learning_rate = 0.001
        self.validation_split_size = 0.118  # ~10% del dataset totale
        self.test_split_size = 0.15  # 15% del dataset totale
        self.pca_components = 30
        self.random_state = 42

def load_client_smartgrid_data_with_validation(client_id, config=None):
    """
    Carica e pre-processa i dati SmartGrid per un client con suddivisione train/validation/test.
    Compatibile con stable_server_standalone.py.
    
    Args:
        client_id: ID del client (1-15)
        config: Configurazione di training (TrainingConfig)
    ...
    print(f"=== CARICAMENTO DATI CLIENT {client_id} SMARTGRID STANDALONE ===")
    ...
    pca = PCA(n_components=min(config.pca_components, X.shape[1]))
    ...
    X_temp, X_test, y_temp, y_test = train_test_split(
        X_pca, y,
        test_size=config.test_split_size,  # 15% per test finale
    ...
    # 10% del totale = validation_split_size del temp set
    ...
    print(f"=== RIEPILOGO CLIENT {client_id} STANDALONE ===")
    ...
    print(f"  - Epoche per round configurate: {config.epochs_per_round}")
    print("=" * 60)
    
    return X_train_scaled, y_train, X_val_scaled, y_val, X_test_scaled, y_test, scaler, pca, dataset_info

def create_smartgrid_client_model_with_validation(input_shape, config=None):
    """
...
class SmartGridClientWithValidationStandalone(fl.client.NumPyClient):
    """
    Client Flower per SmartGrid con supporto train/validation/test split.
    Versione standalone ottimizzata per 10 rounds con 5 epoche per round.
    """
    ...
    def fit(self, parameters, config):
        ...
        ⚠️ CONFIGURAZIONE: 5 epoche per round.
        """
        global model, X_train, y_train, X_val, y_val, dataset_info, training_config
        
        print(f"[Client {client_id}] === ROUND DI ADDESTRAMENTO STANDALONE ===")
        print(f"[Client {client_id}] Ricevuti parametri dal server, avvio addestramento locale...")
        print(f"[Client {client_id}] ⚠️ CONFIGURAZIONE: {training_config.epochs_per_round} epoche per round")
        sys.stdout.flush()
        ...
        print(f"[Client {client_id}] Epoche per round: {training_config.epochs_per_round}")
        
        history = model.fit(
            X_train, y_train,
            validation_data=(X_val, y_val),  # ⚠️ MONITORING: validation set
            epochs=training_config.epochs_per_round,  # ⚠️ CONFIGURAZIONE: 5 epoche
        ...
        print(f"[Client {client_id}] === RISULTATI ADDESTRAMENTO STANDALONE ===")
        ...
        # ⚠️ STABILITÀ: Controlla overfitting
        overfitting_warning = ""
        gap = train_accuracy - val_accuracy
        if gap > 0.1:  # Differenza > 10%
            overfitting_warning = " ⚠️ POSSIBILE OVERFITTING"
        elif gap < -0.05:  # Validation molto meglio del training
            overfitting_warning = " ✅ OTTIMA GENERALIZZAZIONE"
        elif abs(gap) <= 0.05:  # Gap piccolo
            overfitting_warning = " ✅ BUON EQUILIBRIO"
        
        print(f"[Client {client_id}] Gap Train-Val Accuracy: {gap:.4f}{overfitting_warning}")
        
        # Metriche da inviare al server (compatibili con stable_server_standalone.py)
        metrics = {
            # Training metrics
            ...
            # ⚠️ VALIDATION: Validation metrics
            ...
            # ⚠️ STABILITÀ: Overfitting indicator per server tracking
            'overfitting_score': float(gap),
            
            # ⚠️ CONFIGURAZIONE: Training config info
            'epochs_per_round': int(training_config.epochs_per_round),
            'batch_size': int(training_config.batch_size)
        }
        
        print(f"[Client {client_id}] Invio pesi aggiornati al server...")
        print(f"[Client {client_id}] ✅ Addestramento completato con {training_config.epochs_per_round} epoche")
        sys.stdout.flush()
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"[Client {client_id}] === VALUTAZIONE FINALE SU TEST SET STANDALONE ===")
        ...
        print(f"[Client {client_id}] === RISULTATI TEST FINALE STANDALONE ===")
...
def main():
    """
    Funzione principale per avviare il client SmartGrid standalone.
    Compatibile con stable_server_standalone.py (10 rounds, 5 epoche per round).
    """
    global client_id, model, X_train, y_train, X_val, y_val, X_test, y_test, scaler, pca, dataset_info, training_config
    
    # Verifica argomenti della riga di comando
    if len(sys.argv) != 2:
        print("Uso: python stable_client_standalone.py <client_id>")
        print("Esempio: python stable_client_standalone.py 1")
        print("Client ID validi: 1-15 (corrispondenti ai file data1.csv - data15.csv)")
        print("⚠️ NOTA: Compatibile con stable_server_standalone.py (10 rounds)")
        sys.exit(1)
    ...
    print(f"AVVIO CLIENT SMARTGRID STANDALONE {client_id}")
    print("⚠️ CONFIGURAZIONE: Compatibile con 10 rounds, 5 epoche per round")
    print("=" * 50)
    
    try:
        # 0. Inizializza configurazione di training
        training_config = TrainingConfig()
        print(f"[Client {client_id}] Configurazione training:")
        print(f"[Client {client_id}]   - Epoche per round: {training_config.epochs_per_round}")
        print(f"[Client {client_id}]   - Batch size: {training_config.batch_size}")
        print(f"[Client {client_id}]   - Learning rate: {training_config.learning_rate}")
        print(f"[Client {client_id}]   - PCA components: {training_config.pca_components}")
        
        # 1. Carica i dati locali del client con suddivisione train/val/test
        X_train, y_train, X_val, y_val, X_test, y_test, scaler, pca, dataset_info = load_client_smartgrid_data_with_validation(client_id, training_config)
        
        # 2. Crea il modello locale
        print(f"[Client {client_id}] Creazione modello locale...")
        model = create_smartgrid_client_model_with_validation(X_train.shape[1], training_config)
        print(f"[Client {client_id}] Modello creato con {X_train.shape[1]} feature di input")
        
        # 3. Stampa riassunto del client
        print(f"[Client {client_id}] === RIASSUNTO CLIENT STANDALONE ===")
        ...
        print(f"[Client {client_id}] Tentativo di connessione al server standalone su localhost:8080...")
        print(f"[Client {client_id}] ⚠️ COMPATIBILITÀ: Server deve essere stable_server_standalone.py")
        sys.stdout.flush()
        
        # 4. Avvia il client Flower con la classe standalone
        fl.client.start_numpy_client(
            server_address="localhost:8080",
            client=SmartGridClientWithValidationStandalone()  # ⚠️ STANDALONE: classe ottimizzata
        )
