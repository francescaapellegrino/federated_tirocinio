...
class ImprovedSmartGridClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # *** AGGIUNTA: Calcolo AUC ROC, Specificity, Sensitivity sui dati di validation ***
        try:
            ...
            if cm.shape == (2, 2):
                ...
                val_sensitivity = tp / (tp + fn) if (tp + fn) > 0 else 0.0  # Same as recall
            else:
                val_specificity = 0.0
        ...
        # Metriche enhanced con valori garantiti scalari INCLUSE LE NUOVE METRICHE
        metrics = {
            # Training metrics
            ...
            'val_auc_roc': float(val_auc_roc),        # *** AGGIUNTA ***
            'val_specificity': float(val_specificity), # *** AGGIUNTA ***
            'val_sensitivity': float(val_sensitivity), # *** AGGIUNTA ***
