"""
Client Malevolo Compatibile - Inference Attack
Francesca Pellegrino
Compatibile con client.py e server.py attuali
"""
...
# Import per preprocessing compatibile
...
class CompatibleClientConfig:
    """Configurazione compatibile con il sistema attuale"""
    RANDOM_SEED = 42

# CLIENT MALEVOLO COMPATIBILE
class CompatibleMaliciousClient(fl.client.NumPyClient):
    """
    Client malevolo compatibile con l'architettura attuale:
    - Usa improved_preprocessing.py per il caricamento dati
    - Usa improved_model.py per la creazione del modello
    - Architettura [256, 128, 64, 32] con 30 features
    - Compatibile con server.py attuale
    """
    ...
    def __init__(self, client_id: int, is_malicious: bool = True):
        ...
        print(f"🎯 COMPATIBLE MALICIOUS CLIENT {client_id} - {'MALEVOLO' if is_malicious else 'NORMALE'}")
        ...
        # Crea modello compatibile
        self.create_compatible_model()
        
        print(f"✅ Malicious Client {client_id} compatibile inizializzato")
        print(f"   📊 Features: {self.X_train.shape[1]}")
        print(f"   📊 Train: {len(self.X_train)} samples")
        print(f"   📊 Attack ratio: {self.y_train.mean()*100:.1f}%")
    
    def load_compatible_data(self):
        """Carica dati usando il preprocessing migliorato (identico al client normale)"""
        print(f"🔧 Caricamento dati compatibile...")
        
        # USA ESATTAMENTE LO STESSO PREPROCESSING DEL CLIENT NORMALE
        ...
        assert self.X_train.dtype == np.float32, f"Tipo incompatibile: {self.X_train.dtype}"
    ...
    def create_compatible_model(self):
        """Crea modello usando improved_model.py (identico al client normale)"""
        print(f"🔧 Creazione modello compatibile...")
        
        # USA ESATTAMENTE LO STESSO MODELLO DEL CLIENT NORMALE
        self.model = create_improved_model(self.X_train.shape[1], self.config)
        
        # Verifica parametri
        param_count = self.model.count_params()
        print(f"✅ Modello compatibile creato: {param_count:,} parametri")
        print(f"   🏗️ Architettura: [256, 128, 64, 32] → 1")
    ...
    def membership_inference_attack(self):
        """Membership Inference Attack migliorato con tecniche avanzate"""
        print("🔍 Enhanced Membership Inference Attack")
        ...
        try:
            ...
            # Strategia B: Entropia-based (più sofisticata)
            ...
            # 2. ENHANCED LOSS-BASED ATTACK
            print("   📊 Enhanced loss-based analysis...")
            ...
            for loss_type in ['cross_entropy', 'mse', 'focal_loss']:
                if train_losses[loss_type] and test_losses[loss_type]:
                    ...
                    # KS test per distribuzione (più robusto della media)
                    try:
            ...
            # 3. ADVANCED GRADIENT ANALYSIS
            print("   🎯 Advanced gradient analysis...")
            ...
            # 4. ENSEMBLE COMBINATION MIGLIORATA
            print("   🔧 Advanced signal combination...")
    ...
    def property_inference_attack(self):
        """Property Inference Attack ottimizzato"""
        print("🔍 Enhanced Property Inference Attack")
        ...
        try:
            ...
            # 4. ENSEMBLE FUSION
            print("   🔧 Intelligent ensemble fusion...")
            ...
            # 5. ENHANCED PROPERTY DETECTION
            print("   🎯 Enhanced property detection...")
            
            properties_detected = 0
            total_properties = 8  # Aumentato
            detection_results = {}
            ...
            print(f"   📊 Enhanced Property Inference Results:")
    ...
    def find_optimal_clusters(self, data, max_k=8):
        """Trova numero ottimale di cluster usando elbow method compatibile"""
        try:
            if len(data) < max_k:
            ...
            print(f"      ⚠️ Errore find_optimal_clusters: {e}")
            return 3

    def calculate_skewness(self, data):
        """Calcola skewness in modo compatibile"""
        try:
            mean = np.mean(data)
            std = np.std(data)
            if std == 0:
    ...
    def calculate_kurtosis(self, data):
        """Calcola kurtosis in modo compatibile"""
        try:
            mean = np.mean(data)
            std = np.std(data)
            if std == 0:
    ...
    def model_inversion_attack(self):
        """Model Inversion Attack migliorato con tecniche avanzate"""
        print("🔍 Enhanced Model Inversion Attack")
        ...
        try:
            ...
            if high_conf_normal is None or high_conf_attack is None:
                ...
                # Top e bottom 20% invece di numeri fissi
            ...
            # 2. ADVANCED PROTOTYPE GENERATION
            print("   🎯 Advanced prototype generation...")
            ...
            # Pesi esponenziali per enfatizzare alta confidence - VERSIONE CORRETTA
            ...
            # CORREZIONE: Gestisci il caso di pesi zero
            if np.sum(normal_weights) == 0:
                print("      ⚠️ Normal weights sum to zero, using uniform weights")
            ...
            if np.sum(attack_weights) == 0:
                print("      ⚠️ Attack weights sum to zero, using uniform weights")
            ...
            if np.any(np.isnan(normal_weights)) or np.any(np.isnan(attack_weights)):
                print("      ⚠️ NaN weights detected, using uniform weights")
            ...
            # normal_prototypes.append(("exp_weighted", normal_proto_weighted))
            # attack_prototypes.append(("exp_weighted", attack_proto_weighted))

            # 3. ENHANCED GRADIENT-BASED OPTIMIZATION
            print("   ⚙️ Enhanced gradient optimization...")
                ...
                try:
                    ...
                    for i in range(iterations):
                        with tf.GradientTape() as tape:
                            ...
                            # Diversity regularization (incoraggia diversità dai dati originali)
                            if target_class == 1:
                    ...
                    print(f"        ⚠️ Gradient optimization failed: {e}")
            ...
            # 5. ADVANCED SEPARABILITY ANALYSIS
            print("   🔍 Advanced separability analysis...")
            ...
            print(f"   📊 Enhanced Model Inversion Results:")
            ...
            return {
                ...
                "prototype_cosine_similarity": float(1 - cosine_distance),  # Inverti per avere similarity
    ...
    def execute_compatible_attacks(self):
        ...
        try:
            ...
            print(f"⚠️ Livello rischio: {severity}")
            print(f"🔒 FL compromesso: {'SÌ' if successful_attacks >= 2 else 'NO'}")
            print(f"📊 Privacy score: {privacy_risk_score:.3f}")
            print(f"🏗️ Architettura: [256, 128, 64, 32] → 1")
            print(f"🔧 Preprocessing: improved_preprocessing")
            print(f"✅ Compatibilità: CONFERMATA")
    ...
    # METODI FLOWER CLIENT (COMPATIBILI)
    def get_parameters(self, config):
        """Restituisce i parametri del modello"""
        return self.model.get_weights()

    def fit(self, parameters, config):
        """Training del modello (identico al client normale)"""
        print(f"\n[COMPATIBLE MALICIOUS Client {self.client_id}] Training...")
        
        # Imposta pesi dal server
        self.model.set_weights(parameters)
        
        # Callbacks avanzati (identici al client normale)
        callbacks = create_advanced_callbacks(self.config)

        # Class weights (identici al client normale)
        class_weights = compute_class_weight('balanced', classes=np.unique(self.y_train), y=self.y_train)
        class_weight_dict = {0: class_weights[0], 1: class_weights[1]}

        # Training identico al client normale
        history = self.model.fit(
            self.X_train, self.y_train,
            validation_data=(self.X_val, self.y_val),
            epochs=25,  # Identico al client normale
            batch_size=64,  # Identico al client normale
            class_weight=class_weight_dict,
            callbacks=callbacks,
            verbose=1
        )
        
        # Estrazione metriche finali (identica al client normale)
        ...
        # Calcolo AUC ROC, Specificity, Sensitivity (identico al client normale)
        try:
        ...
        print(f"[COMPATIBLE MALICIOUS Client {self.client_id}] Training completato:")
        ...
        # Metriche identiche al client normale
        metrics = {
            # Training metrics
            ...
            # Client info (con indicatore malevolo)
            'client_id': int(self.client_id),
    ...
    def evaluate(self, parameters, config):
        """Valutazione del modello con esecuzione attacchi compatibili"""
        print(f"\n[COMPATIBLE MALICIOUS Client {self.client_id}] Evaluation...")

        # Imposta pesi dal server
        self.model.set_weights(parameters)
        
        # Valutazione standard (identica al client normale)
        results = self.model.evaluate(self.X_test, self.y_test, verbose=0)
        
        # Estrazione sicura dei valori (identica al client normale)
        ...
        # Calcoli aggiuntivi (identici al client normale)
        ...
        print(f"[COMPATIBLE MALICIOUS Client {self.client_id}] Standard Results:")
        ...
        # ESECUZIONE ATTACCHI COMPATIBILI
        attack_results = {}
        if self.is_malicious:
            try:
                print(f"\n🔴 ESECUZIONE ATTACCHI PRIVACY COMPATIBILI...")
        ...
        # Metriche complete per il server (identiche al client normale + info attacchi)
        metrics = {
...
# MAIN FUNCTION COMPATIBILE
def main():
    """Funzione principale del client malevolo compatibile"""
    ...
    try:
        ...
        print("💡 Formato corretto: python3 malicious_client_inference.py <client_id> <true/false>")
        sys.exit(1)

    print(f"\n🚀 === CLIENT MALEVOLO COMPATIBILE {client_id} ===")
    print("=" * 70)
    ...
    print(f"🏗️ Architettura: [256, 128, 64, 32] → 1 (Compatible)")
    print(f"🔧 Preprocessing: improved_preprocessing.py")
    print(f"⚔️ Attacchi: Membership Inference, Property Inference, Model Inversion")
    print(f"✅ Compatibilità: 100% con server.py e client.py attuali")
    print("=" * 70)
    
    if is_malicious:
        print("🔴 ATTENZIONE: Client configurato in modalità MALEVOLA")
        print("⚔️ Attacchi privacy verranno eseguiti durante la valutazione")
    ...
    try:
        ...
        print(f"\n🎉 CLIENT MALEVOLO COMPATIBILE {client_id} TERMINATO!")
        if is_malicious:
            print("📊 Controllare i file JSON generati per i risultati degli attacchi")
